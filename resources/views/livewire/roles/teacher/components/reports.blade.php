<!-- Reports View this is where to select classroom to create report for student -->
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="border rounded-lg p-4 hover:bg-gray-50 cursor-pointer" 
                    ...
                    <h3 class="font-medium text-gray-900">Grade {{ $classroom->grade_level }} - {{ $classroom->section }}</h3>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                    ...
                    <h2 class="text-xl font-semibold text-gray-800">
                        Generate Reports - Grade {{ $selectedClassroom?->grade_level ?? '' }} {{ $selectedClassroom?->section ? ('- ' . $selectedClassroom->section) : '' }}
                    </h2>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @php
                        ...
                        $subjectsQuery = \App\Models\Subject::query()
                            ...
                            ->where('is_active', true);
                        $allSubjects = $subjectsQuery->get();
                        
                        $totalScore = 0;
                        $countedSubjects = 0;
                        foreach($allSubjects as $subject) {
                            // Find the matching student record in the subject's classroom
                            $targetStudentId = $student->id;
                            if ($subject->classroom_id !== $selectedClassroomForReports) {
                            ...
                            $subjectGrades = \App\Models\Grade::where('student_id', $targetStudentId)
                                ->where('is_active', true)
                        ...
                        $averageGrade = $countedSubjects > 0 ? ($totalScore / $countedSubjects) : 0;
                    @endphp
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <button wire:click="openReportScopeModal({{ $student->id }})" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="border-2 border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow bg-gradient-to-br from-blue-50 to-white">
                    ...
                    <div class="flex space-x-2">
                            ...
                            <button wire:click="openDeleteReportCardModal({{ $reportCard->id }})" 
...
@if(isset($confirmingDeleteReportCardId) && $confirmingDeleteReportCardId)
<!-- Delete Report Card Confirmation Modal -->
<div x-data="{ show: @entangle('confirmingDeleteReportCardId').live }" x-cloak x-show="show" class="fixed inset-0 z-[80]">
    <div class="absolute inset-0 bg-black/50" @click="$wire.set('confirmingDeleteReportCardId', null)"></div>
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <h3 class="text-lg font-medium text-gray-900">Confirm Deletion</h3>
                <p class="mt-2 text-sm text-gray-600">Are you sure you want to delete this report card? This action cannot be undone.</p>
            </div>
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button wire:click="confirmDeleteReportCard" 
                        class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                    Delete Report Card
                </button>
                <button wire:click="cancelDeleteReportCard" 
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-gray-500">
                        Grade {{ $this->subjects->where('id', $selectedSubjectForGrades)->first()->classroom->grade_level ?? '' }} - {{ $this->subjects->where('id', $selectedSubjectForGrades)->first()->classroom->section ?? '' }}
                    </p>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    <button wire:click="setView('subjects')" 
