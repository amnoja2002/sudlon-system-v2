<div class="bg-gray-100 min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="md:hidden py-2">
                ...
                >
                    ...
                    <option value="requests" @if($currentView === 'requests') selected @endif>Requests</option>
            ...
            <nav class="hidden md:flex space-x-6 overflow-x-auto no-scrollbar" aria-label="Tabs">
                ...
                <button wire:click="setView('requests')" 
                        class="@if($currentView === 'requests') border-blue-500 text-blue-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                    Requests
                    @php $reqCount = \App\Models\ParentAccessRequest::where('teacher_id', auth()->id())->where('status','pending')->count(); @endphp
                    @if($reqCount > 0)
                        <span class="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs font-medium bg-red-600 text-white rounded-full">{{ $reqCount }}</span>
                    @endif
                </button>
    ...
    @include('livewire.teacher-dashboard.classrooms')
    @include('livewire.teacher-dashboard.students')
    @include('livewire.teacher-dashboard.subjects')
    @include('livewire.teacher-dashboard.attendance')
    @include('livewire.teacher-dashboard.reports')
    @include('livewire.teacher-dashboard.requests')
    ...
    <div x-data="{ open: @entangle('showingRejectModal') }" x-show="open" class="fixed inset-0 z-50 flex items-center justify-center">
        <div class="absolute inset-0 bg-black/40" @click="open=false"></div>
        <div class="relative bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Reject Access Request</h3>
            <label class="block text-sm font-medium text-gray-700 mb-2">Reason</label>
            <textarea wire:model.defer="rejectReason" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" rows="3"></textarea>
            <div class="mt-4 flex justify-end gap-3">
                <button @click="open=false" class="px-4 py-2 rounded-md border">Cancel</button>
                <button wire:click="rejectAccessRequest" class="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700">Reject</button>
            </div>
        </div>
    </div>
