<div class="bg-gray-100 min-h-screen">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="space-y-2">
                <button class="w-full text-left px-4 py-2 text-sm text-blue-600 hover:bg-blue-50 rounded-md transition-colors">
                    Generate Reports
                </button>
                <button class="w-full text-left px-4 py-2 text-sm text-blue-600 hover:bg-blue-50 rounded-md transition-colors">
                    Export Data
                        ...
                        <tr wire:key="user-{{ $user->id }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs rounded-full 
                                    @if($user->role?->slug === 'teacher') bg-green-100 text-green-800
                                    @elseif($user->role?->slug === 'student') bg-blue-100 text-blue-800
                                    @elseif($user->role?->slug === 'parent') bg-purple-100 text-purple-800
                    ...
                    <div class="mt-4">
                        @if($selectedUser)
                            <div class="mb-4">
                                ...
                                <input type="text" wire:model="userData.name" class="mt-1 block w-full rounded-md border-gray-300 px-2">
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700">Email</label>
                                <input type="email" wire:model="userData.email" class="mt-1 block w-full rounded-md border-gray-300 px-2">
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700">Role</label>
                                <select wire:model.live="selectedRole" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 px-2">
                        ...
                        @else
                            ...
                            <div class="mb-4">
                                ...
                                <input type="password" wire:model="userData.password" class="mt-1 block w-full rounded-md border-gray-300 px-2">
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700">Role</label>
                                <select wire:model="userData.role_id" class="mt-1 block w-full rounded-md border-gray-300 px-2">
                                    <option value="">Select Role</option>
                                    @foreach($roles as $role)
                                        <option value="{{ $role->id }}">{{ $role->name }}</option>
                                    @endforeach
