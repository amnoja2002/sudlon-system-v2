...
class AdvancedDashboard extends Component
{
    ...
    // Current view state
    public $currentView = 'dashboard'; // dashboard, classrooms, students, subjects, grades, attendance, reports, requests, profile, subject-grades, excel
    ...
    public function exportClassroomStudents($classroomId)
    {
        ...
        if (!$classroom || $classroom->teacher_id !== auth()->id()) {
            session()->flash('error', 'Classroom not found or access denied.');
            return;
        }

        $students = $classroom->students()->where('is_active', true)->orderBy('name')->get();
        
        $csv = view('excel.student-data-export', compact('students'))->render();

        $filename = 'students-' . ($classroom->display_name ?? "grade-{$classroom->grade_level}-{$classroom->section}") . '-' . now()->format('Y-m-d') . '.csv';
